...
vec3 RGBtoLAlphaBeta(vec3 color) {
  ...
  float alpha = oneOverRootSix * L + oneOverRootSix * M - 2.0 * oneOverRootSix * S;
...
vec3 LAlphaBetaToRGB(vec3 color) {
  ...
  float S = rootThreeOverThree * color.r - 2.0 * rootSixOverSix * color.g;
...
void main()
{
	...
	vec3 color = vec3(2.0 * ambient + 0.6 * diffuse);
