...
namespace ape {
  namespace visualization {
    ...
    bool OgreRay::Raycast(Ogre::Ray& ray, Ogre::uint32 queryMask,
      Ogre::Vector3& resultVec, Ogre::MovableObject** resultObj,
      size_t& resultSubIdx)
    {
      if (!m_raySceneQuery)
        return false;
      ...
      Ogre::RaySceneQueryResult const& query_result = m_raySceneQuery->getLastResults();
      for (size_t qr_idx = 0, size = query_result.size(); qr_idx<size; ++qr_idx)
      {
        Ogre::RaySceneQueryResultEntry const& qr = query_result[qr_idx];
        // stop checking if we have found a raycast hit that is closer
        // than all remaining entities
        if (closest_distance >= 0.0f && closest_distance < qr.distance)
          break;
        ...
        if (qr.movable)
        {
          ...
          std::string const& movableType = qr.movable->getMovableType();
          if (movableType == "ManualObject") {
            Ogre::ManualObject* pentity = static_cast<Ogre::ManualObject*>(
              qr.movable);
            submesh_count = pentity->getNumSections();
          }
          else if (movableType == "Entity") {
            Ogre::Entity *pentity = static_cast<Ogre::Entity*>(qr.movable);
            // get SubMeshCount
          ...
          for (size_t sub_idx = 0; sub_idx < submesh_count; ++sub_idx)
          {
           ...
           /* if (movableType == "ManualObject") {
            ...
            {
              Ogre::Entity *pentity = static_cast<Ogre::Entity*>(qr.movable);
              GetSubMeshInformation(pentity, sub_idx, vertex_count, vertices,
                index_count, indices,
                pentity->getParentNode()->_getDerivedPosition(),
                pentity->getParentNode()->_getDerivedOrientation(),
                pentity->getParentNode()->_getDerivedScale());
            ...
            for (int i = 0; i < index_count; i += 3)
            {
              ...
              if (hit.first && (closest_distance < 0.0f
    ...
    void OgreRay::GetMeshInformation(const Ogre::Entity* entity,
    ...
    void OgreRay::GetMeshInformation(const Ogre::MeshPtr mesh,
      size_t &vertex_count, Ogre::Vector3*& vertices, size_t& index_count,
      unsigned long*& indices, const Ogre::Vector3& position,
      const Ogre::Quaternion& orient, const Ogre::Vector3& scale)
    {
    ...
    void OgreRay::GetMeshInformation(const Ogre::ManualObject* manual,
      size_t& vertex_count, Ogre::Vector3*& vertices, size_t& index_count,
      unsigned long*& indices, const Ogre::Vector3& position,
      const Ogre::Quaternion& orient, const Ogre::Vector3& scale)
    {
      ...
      for (unsigned int i = 0; i < manual->getNumSections(); ++i)
      {
        ...
        //Collect the indices
        {
          if (renderOp->useIndexes) {
            ...
            {
              // No triangles here, so we just drop the collected vertices
              // and move along to the next section.
              continue;
            }
            else {
    ...
    void OgreRay::GetSubMeshInformation(const Ogre::Entity* entity,
      const size_t subMeshIndex, size_t& vertex_count, Ogre::Vector3*& vertices,
    ...
    // Get the mesh information for the given mesh.
    // Code found in Wiki: www.ogre3d.org/wiki/index.php/RetrieveVertexData
    void OgreRay::GetSubMeshInformation(const Ogre::MeshPtr mesh,
      const size_t subMeshIndex, size_t &vertex_count, Ogre::Vector3*& vertices,
      size_t& index_count, unsigned long*& indices,
      const Ogre::Vector3& position, const Ogre::Quaternion& orient,
      const Ogre::Vector3& scale)
    {
      ...
      for (unsigned short i = 0, size = mesh->getNumSubMeshes(); i<size; ++i)
      {
        ...
        // need to check if index buffer is valid (which will be not if the
        // mesh doesn't have triangles like a pointcloud)
        if (ibuf.isNull())
          continue;
    ...
    void OgreRay::GetSubMeshInformation(const Ogre::ManualObject* manual,
      const size_t subMeshIndex, size_t& vertex_count, Ogre::Vector3*& vertices,
      size_t& index_count, unsigned long*& indices,
      const Ogre::Vector3& position, const Ogre::Quaternion& orient,
      const Ogre::Vector3& scale)
